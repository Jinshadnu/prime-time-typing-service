...
export default function HeroSlider({ lang, onNavigateSection }) {
  ...
  return (
    <section className="relative bg-[#0A0B0E] text-white overflow-hidden">
                ...
                <div className="max-w-3xl text-start">
                  ...
                  {isActive && (
                    <>
                      ...
                      {/* CTAs */}
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 animate-hero-cta">
                        <button
                          onClick={() => onNavigateSection('services')}
                          className="btn-gold text-sm md:text-base px-7 py-3.5 sm:py-4 group cursor-pointer shadow-md w-full sm:w-auto justify-center"
                        >
      ...
      {/* Carousel Navigation Controls - White Transparent */}
      <div className="absolute z-30 bottom-3 right-3 sm:bottom-6 sm:right-6 md:right-12 left-auto flex items-center gap-2 sm:gap-3 bg-white/90 backdrop-blur-xl px-3 sm:px-5 py-2 sm:py-2.5 rounded-full border border-slate-200/80 shadow-xl max-w-[calc(100vw-1.5rem)]">
        <button
          onClick={() => setIsPlaying(!isPlaying)}
          className="text-slate-800 hover:text-[#8C6A21] p-1.5 transition-colors cursor-pointer"
          ...
          {slides.map((_, idx) => (
            <button
              ...
              className={`h-2.5 rounded-full transition-all duration-300 cursor-pointer ${
                idx === currentSlide ? 'w-8 bg-[#D4AF37] shadow-sm' : 'w-2.5 bg-slate-300 hover:bg-slate-400'
              }`}
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="text-slate-800 hover:text-[#8C6A21] p-1 rounded-full hover:bg-slate-100 transition-colors cursor-pointer"
